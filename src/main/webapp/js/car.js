...
$(function(){
	...
	$.get("../cart/finds", null, function(data){
		...
		$.each(data, function(index, item) {
			str += '<ul><li class="col01"><input type="checkbox" value="'+item.cno+'" checked></li><li class="col02"><img src="../../'+item.pics.split(";")[0]+'"></li>';
		...
		for (var i = 0; i < len; i ++) { // 循环给每一个绑定
			checkboxs[i].onclick = function() {
				productCount(); // 重新计算总价等信息
				
				// 判断有没有没有选择的，如果有则全选取消，否则全选选中
				for (var j = 0; j < len; j ++) {
					if (!checkboxs[j].checked) { // 只要有一个没选择，则全选就不能选中
...
// 去结算，首先需要添加一条订单信息，然后可能要添加多条订单详细信息，还需要删除购物车信息  -> 首先必须获取用户选中的商品
function gotoPay() {
	var strs = [];
	...
	$.post("../order/order", {cnos:strs.join(";")}, function(data) {
		data = parseInt($.trim(data));
		if (data > 0) {
			location.href="pay.html"; // 到支付界面
		} else {
			alert("下单失败，请稍后重试...");
		}
	}, "text");
